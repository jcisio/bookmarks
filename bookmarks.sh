...
bookmarks ()
{
  ...
  # Get the script directory to read the README file, also resolve symlinks.
  ...
  if [ $# -eq 0 ] ; then
    printf "\nDIRECTORY BOOKMARKS FOR BASH (version %s)\n\n" $BBM_Version
    sed -n '/^BOOKMARK/,/^REGEX/p' $DIR/README.md | sed '1,11d' | sed '$d'
    return 0
  fi
